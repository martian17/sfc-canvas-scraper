...
(async function(){
    ...
    let kenkyu = classes.filter((c,i)=>{
        try{
            //return c.name.match("Ａ");
            return c.name.match("建築");
        }catch(err){
            console.log(c,i);
            throw err;
        }
    ...
    //.map(c=>c.name);
    console.log(kenkyu);
    console.log(kenkyu.map(c=>[c.course_code,c.name,c.public_description]));
    
    let view = new ELEM(document.getElementById("viewport"));
    console.log(kenkyu.map(c=>{
